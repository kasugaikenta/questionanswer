...
<div class="question_detail_container">
    ...
    <div class="container">
        @foreach ($question->answers as $answer)
            <div class="row">
                <div id="answer-card" class="col-md-8 offset-md-2 answer_wrapper">
                    <div class="answer_list">
                        <!-- 回答は匿名 -->
                        <div class="answer_content">{{ $answer->content }}</div>
                        <div id="option" class="answer_time">{{ $answer->created_at }}</div> <!-- 「回答日時：」って文字入れる？ -->
                    </div>
                </div>
            </div>
        @endforeach
    ...
    <div class="btnWrapper">
        <a id="answer-button" class="btn" href="{{ url('/answer', $question->id) }}">回答する</a>
    </div>
</div>
@endsection
